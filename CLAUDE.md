...
**Version:** v3.0.0 (`VERSION` file → Dockerfile → `deps.py:_read_version()` → `/api/health`). CI writes git tag into `VERSION`. Frontend compares to latest GitHub release for update banner.
